...
public final class URLocation {
	...
	public String buildUnescapedURL(){
		...
		if (hasServer()){
			url.append("//");
			if (hasUserInfo()){
				url.append(unescape(userInfo,encoding)).append("@");
			}
			url.append(unescape(server,encoding));
			if (hasPort()){
				url.append(":").append(port);
			}
		}
		
		url.append(unescape(path,encoding));
		
		if (hasQuery()){
			url.append("?").append(unescape(query,encoding));
		}
		
		if (hasFragment()){
			url.append("#").append(unescape(fragment,encoding));
		}
